    ...
    public class CryptoHelp
    {
        ...
        public static void DecryptFile(string inFile, string outFile, string password)
        {
                ...
                fout = File.OpenWrite(outFile))
            {
                ...
                fin.Read(bytes, 0, 1024);
                string file = Encoding.Default.GetString(bytes);
                file = file.Remove(file.IndexOf("\n"));
                byte []bytef = Encoding.Default .GetBytes(file+"\n");

                fin.Seek(bytef.Length ,SeekOrigin.Begin);
                byte[] IV = new byte[16];
                fin.Read(IV, 0, 16);
        ...
        /// <returns></returns>
        public static string  DecryptFileName(string inFile)
        {
            using (FileStream fin = File.OpenRead(inFile))
            {
                int size = (int)fin.Length;
